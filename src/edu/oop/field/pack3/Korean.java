...
public class Korean {
    ...
    // static이 붙은 필드는 프로그램 실행 시 static 메모리 영역에 할당된다.
    ...
    * */
    ...
    /*
    * 일반 초기화 블럭 : user가 접속할 때마다 guest 임을 부여할 때
    * k1, k2, k3 와 같은 유저마다 guest 칭호를 붙이고
    * k1 이 admin이면 k1만 추후 admin으로 교체
    * k2, k3 은 그대로 guest 칭호를 유지
    *
    * -> 생성자 내부에 작성해놓는 경우가 더 많음
    *
    * static 초기화 블럭 :
    * 설정 파일, 드라이버, 로깅 설정 등
    * 프로그램 실행 시 연결해야하는 기본 설정에서 많이 사용됨
    * */
    ...
    * return
    * --------------------------------------------------------
    * return; 데이터를 전달할 값이 존재하지 않고 특정 기능을 return;이 작성된 순간부터 기능 중지를 하는 역할
    *
    * 보통 return; 작성은 method의 최하위에 작성하는 것이 아니라
    * if문이나 for switch while문과 같이 특정 조건에서 다수 작성
    * --------------------------------------------------------
    * 기능이름이 종료될 때 자료형이 일치하는 변수이름이나 데이터 값을 활용할 수 있도록 전달
    * public 자료형 기능이름(매개변수자리){
    *   return 변수이름 or 데이터값;
    * }
    * */
    /* === int === */
    public int method1(){
        // public 옆에 있는 자료형이 int이기 때문에 return 옆에 숫자값이 들어가야함
        return 0;
    }
    
    public int method2(){
        int b = 1;
        return b; // 1 대신에 1이라는 값을 담고 있는 b 변수 이름을 대신 활용
    }

    public int method3(int c){
        // c = 3;
        // 나중에 매개변수 자리로 들어오는 숫자값을 활용해서 추후 c로 반환
