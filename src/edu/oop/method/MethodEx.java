...
* 간단히 (기능 / 변수) 을(를) 사용할 수 있고, 사용 후 메모리에서 정리되는 (기능 / 변수)
* static이 없는 일반 기본 메서드
* 객체를 생성한 후 객체에 붙은 변수 이름을 통해 호출
* 각 객체별로 개별적인 기능 수행
* */
...
public class MethodEx {
    ...
    // 클래스파일명칭.static기능 명칭(매개변수 존재 유무)
    ...
    // MethodEx.systemInfo(); 형식으로 작성해야한다!!
    
    /*
    * 메서드 오버로딩(Method Overloading)
    * 하나의 클래스 내부에서 이름이 똥일한 메서드를 여러 개 정의
    * 같은 이름으로 새로운 메서드를 계속 추가하는 것
    *
    * 오버로딩 조건
    * 메서드 이름은 같아야하지만, 매개변수는 달라야 함
    ...
    *
    * 생성자는 오버 로딩에 해당
    * 생성자는 - 기본 생성자 : 매개변수 존재하지 않는 생성자
    *                       작성하지 않아도 기본적으로 존재하나 매개변수 생성자가 생성되면
    *                       기본 생성이 소멸되기 때문에, 작성해주어야함
    ...
    * */
    ...
    // 대신 SNS를 통해 본인인증을 마쳤으니 믿고 입장해도 좋다는 의미로 토큰을 제공
    // 축제나 클럽에 들어갈 때 입구에서 신분증을 보여주고 본인인증을 하면
    // 다시는 신분증을 꺼낼 필요 없이 팔찌만 보여주고 자유롭게 다닐 수 있는 것처럼
    // 온라인에서는 팔찌 대신 토큰 제공

    // 6-2 토큰(String)으로 로그인
    public void login(String socialToken){ // 로그인 확인값
        // 만약에 이메일이 들어오면 이메일 확인하고 로그인처리
        // 만약에 토큰이 들어오면 토큰 확인하고 로그인 처리
        System.out.println("[소셜로그인]토큰으로 간편 로그인 성공!");
    }

    // 6-3 유저아이디(int)와 비밀번호(String) 로그인
