...
/*
* 계좌 클래스
* 추후 데이터 베이스(DB)와 자바(Java)를 연결할 변수 명칭을 작성하는 클래스
* */
public class Account {
    // [캡슐화]
    // - 객체의 속석(필드) 직접 접근을 제한하는 것이 원칙
    ...
    // 단축키 Alt + Insert를 클릭하면 생성자 getter setter 를 만들어주는 기능 실행
    // 메서드(기능) - 생성자
    // 생성자 - 기본
    // 생성자 - 필수
    ...
    * private String name;
    *
    * public void setName(String html에서 데이터 전달 받아올 변수이름) {
    *     this.name = html에서 데이터 전달 받아올 변수이름;
    * }
    *
    * setName 은 private String name 변수 명칭에 데이터를 저장하기 위한 메서드
    * this.name = 위에서 작성한 private String name 가리킴
    *
    * this.name = name에서 name은 String name에서 가져온 변수명칭을 가리킴
    *
    * (String name) 매개변수 내부에 존재하는 변수이름은 private에서 작성한 변수 이름과 다르게 작성해도 상관없지만,
    * pricate으로 작성한 속성 명칭이 많아지면 변수 이름들을 관리하는데 문제가 발생할 수 있기 때문에
    ...
    * */
    ...
    // 메서드(기능) - getter : DB나 이메일 인증키와 같은 데이터를 사용자에세 전달하는 역할
    /*
    * @return : 메서드 내부에서 작성한 특정 데이터나 setter로 저장되어있거나,
    ...
    *
    * @param pw : 클라이언트가 작성한 비밀번호를 가지고 있는 매개변수 이름
