...
public class SetService {
    ...
    public void method1(){
        // List ArrayList와 동일하고, List 에서 Set으로 변경되고 중복이 안 되는 것 일 뿐!
        Set<String> set = new HashSet<>();

        // 1. boolean add(E e) :
        // 값을 넣고 true false 형태로 지니는 기능
        set.add("카카오");
        set.add("네이버");
        set.add("라인");
        set.add("당근");
        ...
        System.out.println("set 공간에 들어갈 내용 확인하기 : " + set);
        ...
        // Iterator = 순차적으로 접근 할 수 있도록 설정된 문서
        // 반복자 - 컬렉션에 객체에 저장된 요소를 하나씩 순차 접근하는 객체
        ...
        while(it.hasNext()){
            // set이라는 공간에 다음으로 보여줄 데이터가 존재하면 진행 없으면 멈춤!
    ...
    public void method2(){
        ...
        System.out.println("과자리스트 모두 잘 지우셨나요? : " + 과자들.isEmpty()); // ture false
    }
    ...
    public void lotto(){
        ...
        while(lotto.size() < 6){
            // random.nextInt(45) // 0 < X < 45 의 알 수 없는 수 생성
            lotto.add(random.nextInt(45) + 1);
            // Random 클래스 안쓰고, Math 클래스를 사용했다면
            // lotto.add((int)Math.random() * 45 + 1);
        }
        System.out.println("이번 주 lotto : " + lotto);
    }
}
