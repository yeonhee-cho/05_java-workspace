...
public class LoopEx { // <html>과 같이 페이지 내 코드 시작을 나타내는 구문
    ...
    * */
    public void method1(){
        ...
        * 조건식 : 최초 1회 변수 이름의 데이터 기반으로 참/거짓인지 판별하는 구문
        * 참일 경우 중괄호 내에 존재하는 코드 구문 수행
        * 거짓일 경우 중괄호 탈출
        * 증감식 : 중괄호 내에 존재하는 구문을 수행하고 조건식을 판별하기 전에
        * 변수 이름의 데이터를 증가하거나 감소하는 구문
        * */
        // 초기식 조건식 증감식
        for (int i = 0; i < 10; i++) {
    ...
    * */
    ...
    /*
    * 코드에 대한 설명 작성 시 사용하는 주석
    * 두 수를 입력받아 1씩 증가하며 반복 출력하기
    * - 입력받은 두 수에서 작은 값이 초기식, 큰 값이 조건식에 사용되게 작성
    ...
    * 
    * 사용자로부터 두 수를 입력받고, 
    * 입력받은 수 중에 작은 값과 큰 값을 반환
    * 최소값부터 죄대값까지 for문을 이용하여 출력
    * 
...
* html 코드를 벗어난 상태에서 태그를 작성하거나 텍스트를 작성하더라도 문제가 발생하지 않음
*
