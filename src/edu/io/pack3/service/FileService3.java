...
public class FileService3 {
    public void 고전방식(){
        ...
        try {
            ...
            String line;
            // 가져온 파일에 있는 글자들이 null 값일 때까지 한 줄 씩 읽고 출력
            ...
            if(br != null) {
                try {
                    br.close(); // 모두 읽은 파일을 닫을 때 문제가 발생할 수 있으니 try catch로 시도한 후 발생한 문제에 대해 처리 로직을 catch에 작성
                } catch (Exception e) {
                    System.out.println("파일이 다른 곳에서 사용 중이기 때문에 닫을 수 없습니다.");
                }
            }
        }
    }
    ...
    public void 현대방식(){
        ...
        try {
            ...
            System.out.println("파일을 읽는 도중에 발생하는 문제에 대해 처리할 로직을 작성하는 곳 ");
        }
    }
}
